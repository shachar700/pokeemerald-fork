...
u8 AddStartMenuWindow(u8 numActions)
{
    if (sStartMenuWindowId == WINDOW_NONE)
        sStartMenuWindowId = AddWindowParameterized(0, 22, 1, 7, (numActions * 2) + 2, 15, 0x139); //s7 the start menu
...
void RedrawMenuCursor(u8 oldPos, u8 newPos)
{
    ...
    FillWindowPixelRect(sMenu.windowId, PIXEL_FILL(1), sMenu.left + 26, sMenu.optionHeight * oldPos + sMenu.top, width, height);
    AddTextPrinterParameterized(sMenu.windowId, sMenu.fontId, gText_SelectorArrow3, sMenu.left + 32, sMenu.optionHeight * newPos + sMenu.top, 0, 0);
}
...
{
    u32 i;
    // s7 print options in menu table
    for (i = 0; i < itemCount; i++)
        AddTextPrinterParameterized(windowId, 1, menuActions[i].text, 20, (i * 16) + 1, TEXT_SKIP_DRAW, NULL);
