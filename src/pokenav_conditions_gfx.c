...
static bool32 UpdateConditionGraphMenuWindows(u8 mode, u16 bufferIndex, bool8 winMode)
{
    ...
    case 1:
        if (GetConditionGraphCurrentListIndex() != GetMonListCount() - 1 || IsConditionMenuSearchMode() == TRUE)
        {
            ...
            AddTextPrinterParameterizedWithRTL(menu->nameGenderWindowId, FONT_NORMAL, str, 0, 1, 0, NULL, TRUE);
        }
        break;
    case 2:
        if (IsConditionMenuSearchMode() == TRUE)
        {
            ...
            AddTextPrinterParameterizedWithRTL(menu->nameGenderWindowId, FONT_NORMAL, str, 0, 17, 0, NULL, TRUE);
            ...
            AddTextPrinterParameterizedWithRTL(menu->listIndexWindowId, FONT_NORMAL, text, 4, 1, 0, NULL, TRUE);
            ConvertIntToDecimalStringN(&text[5], GetConditionMonDataBuffer(), STR_CONV_MODE_RIGHT_ALIGN, 4);
            AddTextPrinterParameterizedWithRTL(menu->listIndexWindowId, FONT_NORMAL, text, 28, 1, 0, NULL, TRUE);
        }
        break;
    case 3:
