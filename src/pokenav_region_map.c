...
static const u32 sRegionMapCityZoomTiles_Gfx[] = INCGFX_U32("graphics/pokenav/region_map/zoom_tiles.png", ".4bpp.lz");
...
static void UpdateMapSecInfoWindow(struct Pokenav_RegionMapGfx *state)
{
    ...
    case MAPSECTYPE_CITY_CANFLY:
        ...
        AddTextPrinterParameterizedWithRTL(state->infoWindowId, FONT_NARROW, regionMap->mapSecName, 0, 1, TEXT_SKIP_DRAW, NULL, TRUE);
...
static void PrintLandmarkNames(struct Pokenav_RegionMapGfx *state, mapsec_s32_t mapSecId, int pos)
{
    ...
    while (1)
    {
        ...
        StringCopyPadded(gStringVar1, landmarkName, CHAR_SPACE, 12, FALSE);
        AddTextPrinterParameterizedWithRTL(state->infoWindowId, FONT_NARROW, gStringVar1, 0, i * 16 + 17, TEXT_SKIP_DRAW, NULL, TRUE);
        i++;
    }
}
