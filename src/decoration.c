...
static void PrintDecorationCategoryMenuItems(u8 taskId)
{
    ...
    AddTextPrinterParameterizedWithRTL(windowId, FONT_NORMAL, gTasks[taskId].tDecorationMenuCommand == DECOR_MENU_TRADE ? gText_Exit : gText_Cancel, 8, i * 16 + 1, 0, NULL, TRUE);
    ScheduleBgCopyTilemapToVram(0);
}
...
static void DecorationItemsMenu_PrintDecorationInUse(u8 windowId, u32 itemIndex, u8 y)
{
    if (itemIndex != LIST_CANCEL)
    {
        if (IsDecorationIndexInSecretBase(itemIndex + 1) == TRUE)
            BlitMenuInfoIcon(windowId, MENU_INFO_ICON_BALL_RED, 5, y + 2);
        else if (IsDecorationIndexInPlayersRoom(itemIndex + 1) == TRUE)
            BlitMenuInfoIcon(windowId, MENU_INFO_ICON_BALL_BLUE, 5, y + 2);
    }
}
...
static void PrintDecorationItemDescription(s32 itemIndex)
{
    ...
    AddTextPrinterParameterizedWithRTL(windowId, FONT_NORMAL, str, 0, 1, 0, 0, TRUE);
}
