...
{
    AddTextPrinterParameterized4WithRTL(windowId, FONT_NORMAL, x, y, 0, 0, sTextColors[colorStructId], speed, text, TRUE);
}
...
static void PrintBerrySize(void)
{
    ...
    {
        AddTextPrinterParameterizedWithRTL(WIN_SIZE_FIRM, FONT_NORMAL, gText_ThreeMarks, 0x28, 1, 0, NULL, TRUE);
    }
}
...
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterizedWithRTL(WIN_SIZE_FIRM, FONT_NORMAL, gText_FirmSlash, 0, 0x11, TEXT_SKIP_DRAW, NULL, TRUE);
    if (berry->firmness != 0)
        AddTextPrinterParameterizedWithRTL(WIN_SIZE_FIRM, FONT_NORMAL, sBerryFirmnessStrings[berry->firmness - 1], 0x28, 0x11, 0, NULL, TRUE);
    else
        AddTextPrinterParameterizedWithRTL(WIN_SIZE_FIRM, FONT_NORMAL, gText_ThreeMarks, 0x28, 0x11, 0, NULL, TRUE);
}

static void PrintBerryDescription1(void)
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterizedWithRTL(WIN_DESC, FONT_NORMAL, berry->description1, 0, 1, 0, NULL, TRUE);
}

static void PrintBerryDescription2(void)
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterizedWithRTL(WIN_DESC, FONT_NORMAL, berry->description2, 0, 0x11, 0, NULL, TRUE);
}
