...
static void AskUsePokeblock(void)
{
    ...
    AddTextPrinterParameterizedWithRTL(WIN_TEXT, FONT_NORMAL, gStringVar4, 0, 1, 0, NULL, TRUE);
...
static void PrintWontEatAnymore(void)
{
    FillWindowPixelBuffer(WIN_TEXT, 17);
    DrawTextBorderOuter(WIN_TEXT, 151, 14);
    AddTextPrinterParameterizedWithRTL(WIN_TEXT, FONT_NORMAL, gText_WontEatAnymore, 0, 1, 0, NULL, TRUE);
    PutWindowTilemap(WIN_TEXT);
    CopyWindowToVram(WIN_TEXT, COPYWIN_FULL);
}
...
static void UpdateMonInfoText(u16 loadId, bool8 firstPrint)
{
    ...
    if (sMenu->info.curSelection != sMenu->info.numSelections - 1)
    {
        AddTextPrinterParameterizedWithRTL(WIN_NAME, FONT_NORMAL, sMenu->monNameStrings[loadId], 0, 1, 0, NULL, TRUE);
        ...
        AddTextPrinterParameterized3WithRTL(WIN_NATURE, FONT_NORMAL, 2, 1, sNatureTextColors, 0, sMenu->info.natureText, TRUE);
    }
