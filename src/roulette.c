...
static void Task_AskKeepPlaying(u8 taskId)
{
    ...
    DrawStdWindowFrame(sTextWindowId, FALSE);
    AddTextPrinterParameterizedWithRTL(sTextWindowId, FONT_NORMAL, Roulette_Text_KeepPlaying, 0, 1, TEXT_SKIP_DRAW, 0, TRUE);
    CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
...
static void Task_PrintSpinResult(u8 taskId)
{
    ...
    case 2: // never happens
        if (gTasks[taskId].tMultiplier == MAX_MULTIPLIER)
        {
            ...
            DrawStdWindowFrame(sTextWindowId, FALSE);
            AddTextPrinterParameterizedWithRTL(sTextWindowId, FONT_NORMAL, Roulette_Text_Jackpot, 0, 1, TEXT_SKIP_DRAW, NULL, TRUE);
            CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
        }
        else
        {
            ...
            DrawStdWindowFrame(sTextWindowId, FALSE);
            AddTextPrinterParameterizedWithRTL(sTextWindowId, FONT_NORMAL, Roulette_Text_ItsAHit, 0, 1, TEXT_SKIP_DRAW, NULL, TRUE);
            CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
        }
        break;
    case FALSE:
    default:
        ...
        DrawStdWindowFrame(sTextWindowId, FALSE);
        AddTextPrinterParameterizedWithRTL(sTextWindowId, FONT_NORMAL, Roulette_Text_NothingDoing, 0, 1, TEXT_SKIP_DRAW, NULL, TRUE);
        CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
        break;
    }
    gTasks[taskId].data[1] = 0;
...
static void Task_TryPrintEndTurnMsg(u8 taskId)
{
    ...
    if (gTasks[taskId].tCoins >= sRoulette->minBet)
    {
        if (gTasks[taskId].tBallNum == BALLS_PER_ROUND)
        {
            ...
            DrawStdWindowFrame(sTextWindowId, FALSE);
            AddTextPrinterParameterizedWithRTL(sTextWindowId, FONT_NORMAL, Roulette_Text_BoardWillBeCleared, 0, 1, TEXT_SKIP_DRAW, NULL, TRUE);
            CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
        ...
        else if (gTasks[taskId].tCoins == MAX_COINS)
        {
            ...
            DrawStdWindowFrame(sTextWindowId, FALSE);
            AddTextPrinterParameterizedWithRTL(sTextWindowId, FONT_NORMAL, Roulette_Text_CoinCaseIsFull, 0, 1, TEXT_SKIP_DRAW, NULL, TRUE);
            CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
    ...
    else
    {
        // Player out of coins
        DrawStdWindowFrame(sTextWindowId, FALSE);
        AddTextPrinterParameterizedWithRTL(sTextWindowId, FONT_NORMAL, Roulette_Text_NoCoinsLeft, 0, 1, TEXT_SKIP_DRAW, NULL, TRUE);
        CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
...
static void Task_PrintMinBet(u8 taskId)
{
    if (JOY_NEW(A_BUTTON | B_BUTTON))
    {
        ...
        AddTextPrinterParameterizedWithRTL(0, FONT_NORMAL, gStringVar4, 0, 1, TEXT_SKIP_DRAW, NULL, TRUE);
...
static void Task_PrintRouletteEntryMsg(u8 taskId)
{
    ...
    if (gTasks[taskId].tCoins >= minBet)
    {
        if ((gSpecialVar_0x8004 & ROULETTE_SPECIAL_RATE) && (gSpecialVar_0x8004 & 1))
        {
            ...
            DrawStdWindowFrame(0, FALSE);
            AddTextPrinterParameterizedWithRTL(0, FONT_NORMAL, Roulette_Text_SpecialRateTable, 0, 1, TEXT_SKIP_DRAW, NULL, TRUE);
            CopyWindowToVram(0, COPYWIN_FULL);
